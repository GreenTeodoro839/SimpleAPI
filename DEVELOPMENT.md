...
上游提供商配置项。每个 provider 只有一套 `name`、`type`、`url`、`headers`、`key`。`name` 和 `url` 必填，`name` 不能包含 `/`。
...
解析时按第一个 `/` 切分。因为 provider `name` 禁止包含 `/`，所以 `aliasA` 可被完整保留。
...
5. 构建模型索引：`providerName/aliasA -> provider model`。
...
anthropic_web_search_forward:
  enabled: true
  target_model: "providerName/aliasA"
```
...
- internal model id：`providerName/aliasA`
- `aliasA`
- 上游真实模型名
...
- 客户端使用 `aliasB` 请求，服务能路由到 `providerName/aliasA` 并改写成真实模型。
- 同协议透传。
- 跨协议进入翻译层。
